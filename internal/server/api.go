...
import (
	...
	"github.com/Snipa22/go-tari-explorer/docs"
...
// ---- GET /api/spec, GET /api/docs ----

// handleAPISpec serves GET /api/spec: the contents of docs/openapi.yaml (embedded
// into the binary via docs.OpenAPISpecYAML - see that package's own doc comment for
// why the embed lives there rather than directly in this package), served as-is
// (Content-Type application/yaml).
//
// ?format=json transcodes the same spec to JSON instead, using the yaml.v3 library
// (already a transitive dependency of this module via pgx's own test deps, per
// DISPATCH_BRIEF_OPENAPI.md's "skip the JSON transcoding feature entirely if it
// would require a genuinely new dependency" instruction, which this satisfies since
// no new dependency is added - see go.mod). A transcoding failure (which would only
// happen if the embedded YAML were malformed - a build-time programming error, not
// a runtime condition) falls back to a 500 JSON error rather than serving broken
// output.
func (s *Server) handleAPISpec(w http.ResponseWriter, r *http.Request) {
	if r.URL.Query().Get("format") == "json" {
		...
		if err := yaml.Unmarshal(docs.OpenAPISpecYAML, &doc); err != nil {
			log.Printf("server: api: spec: unmarshal embedded yaml: %v", err)
			writeJSONError(w, http.StatusInternalServerError, "failed to transcode spec to json")
			return
		}
	...
	if _, err := w.Write(docs.OpenAPISpecYAML); err != nil {
		log.Printf("server: api: spec: write response: %v", err)
	}
}

// handleAPIDocs serves GET /api/docs: a minimal static HTML page embedding Swagger
// UI via a CDN script tag (templates/api_docs.html - see that file's own comment),
// pointed at /api/spec as its spec URL.
func (s *Server) handleAPIDocs(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.WriteHeader(http.StatusOK)
	if err := s.apiDocsTmpl.Execute(w, nil); err != nil {
		log.Printf("server: render api docs: %v", err)
	}
}
